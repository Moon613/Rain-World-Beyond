...
class FisherMask : PlayerCarryableItem, IDrawable
{
	public FisherMaskAbstract AbstrMsk
	{
		get
		{
			return abstractPhysicalObject as FisherMaskAbstract;
		}
	}
    public FisherMask(FisherMaskAbstract abstractPhysicalObject) : base(abstractPhysicalObject)
    {
		...
		waterFriction = 0.98f;
		buoyancy = 0.6f;
		GenerateColor(AbstrMsk.colorSeed);
    }
    public void GenerateColor(int colorSeed)
    {
        Random.State state = Random.state;
        Random.InitState(colorSeed);
        ColorB = new HSLColor(Mathf.Lerp(150f/360f, 190f/360f, Random.value), Mathf.Lerp(0.5f, 0.75f, 1f - Random.value * Random.value), Mathf.Lerp(0.45f, 0.6f, Random.value * Random.value));
        ColorA = new HSLColor(ColorB.hue, ColorB.saturation + Mathf.Lerp(-0.1f, 0.15f, Random.value), ColorB.lightness + Mathf.Lerp(0.09f, 0.23f, Random.value));
        Random.state = state;
    }
    ...
    public void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
    {
		...
		if (num > 0f && grabbedBy.Count > 0 && grabbedBy[0].grabber is Player player)
		{
			...
			if (playerGraphics != null)
			{
				...
				if (num2 != 0f)
				{
					...
					vector5 += vector4 * 2f * Mathf.Abs(num2);
					vector5 -= Custom.PerpendicularVector(vector4) * 4f * num2;
				}
        ...
        {
			string element = "KrakenMask";
			if (i == 3) { element = "FisherDrop"; }
			if (i == 4) { element = "FisherMark"; }
        ...
        color = Color.Lerp(Color.Lerp(ColorA.rgb, new Color(1f, 1f, 1f), 0.35f * fallOffVultureMode), blackColor, Mathf.Lerp(0.2f, 1f, Mathf.Pow(num5, 2f)));
        sLeaser.sprites[0].color = color;
        sLeaser.sprites[1].color = Color.Lerp(color, blackColor, Mathf.Lerp(0.75f, 1f, num5));
        sLeaser.sprites[2].color = Color.Lerp(color, blackColor, Mathf.Lerp(0.75f, 1f, num5));
        sLeaser.sprites[3].color = Color.Lerp(Color.Lerp(Color.Lerp(HSLColor.Lerp(ColorA, ColorB, 0.8f - 0.3f * fallOffVultureMode).rgb, blackColor, 0.53f), Color.Lerp(ColorA.rgb, new Color(1f, 1f, 1f), 0.35f), 0.1f), blackColor, 0.6f * num5);
        sLeaser.sprites[4].color = Color.Lerp(Color.Lerp(Color.Lerp(HSLColor.Lerp(ColorA, ColorB, 0.8f - 0.3f * fallOffVultureMode).rgb, blackColor, 0.53f), Color.Lerp(ColorA.rgb, new Color(1f, 1f, 1f), 0.35f), 0.1f), blackColor, 0.6f * num5);
